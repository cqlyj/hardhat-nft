  ...
  : describe("Random IPFS NFT Unit Tests", function () {
      ...
      describe("requestNft", () => {
        it("fails if payment isn't sent with the request", async function () {
          try {
            ...
            assert(
              error.message.indexOf("RandomIpfsNft__NeedMoreETHSent") >= 0,
              "Expected revert error not received"
            );
          }
        });
        it("reverts if payment amount is less than the mint fee", async function () {
          const fee = await randomIpfsNft.getMintFee();
          const paymentAmount = fee - ethers.parseEther("0.001");

          try {
            await randomIpfsNft.requestNft({ value: paymentAmount });
            assert.fail("Transaction should have reverted");
          } catch (error) {
            assert(
              error.message.indexOf("RandomIpfsNft__NeedMoreETHSent") >= 0,
      ...
      describe("getBreedFromModdedRng", () => {
        ...
        it("should revert if moddedRng > 99", async function () {
          try {
            await randomIpfsNft.getBreedFromModdedRng(100);
            assert.fail("Transaction should have reverted");
          } catch (error) {
            assert(
              error.message.indexOf("RandomIpfsNft__RangeOutOfBounds") >= 0,
              "Expected revert error not received"
            );
          }
        });
